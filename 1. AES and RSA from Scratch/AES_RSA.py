...
# Functions in case we want a byte array as I/O instead of a string, which is the default case right now

def string_to_byte_array(the_string):

	res_byte = bytearray()
	res_byte.extend(the_string)
...
def byte_array_to_string(the_byte_array):
	...
	for i in xrange(len(the_byte_array)):
		str_res = str_res + chr(the_byte_array[i])
...
def aes_key_generation(key_length):
	...
	key_down = pow(2, key_length-1)

	key_up = (pow(2, key_length)-1)
...
def pick_key():
	...
	print 'Press 4 to select a key from the aes_file'
	...
	if(little_choice==1):
		...
		the_key = file_read_line("aes_file.txt")
...
def my_main():
	...
	while(option!=0):
		...
		print 'Press 3 to select RSA - Encrypt / Decrypt'
		print 'Press 4 to select SHA-2'
		print 'Press 5 to select Digital Signature Sign / Verify'
		...
		if(option==1):
			...
			if(extra_option_0==1):
				...
				if(mode=='ECB' or mode=='ecb'):
					...
					while(len(str(i_v))<32):
						i_v = input("Please enter a correctly sized initialization vector: ")
				...
				if(new_mode=='ECB' or new_mode=='ecb'):
					...
					print ECB_decrypt(aes_data, aes_key)
					...
					print CBC_decrypt(aes_data, aes_key, i_v)
			...
			key_pair_res = key_generation(input("Please enter the length: "))
			...
			if(input("Press 1 if you wish to automatically use the generated keys for options 3 and 5: ")==1):
				e = key_pair_res[0][0]
				n = key_pair_res[1][1]
				d = key_pair_res[1][0]
				flag_option = 1
			...
			print generate_hashed_password(pw)
